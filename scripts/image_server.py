...
def get_precision(device, precision):
    ...
    if "cuda" in device and torch.cuda.is_available() and not precision == "fp32":
        ...
        # If GPU is nvidia 16xx use float16 and enable benchmark mode
            ...
            if False: #not gpu_name.startswith("NVIDIA GeForce GTX 1650") or not gpu_name.startswith("NVIDIA GeForce GTX 1660"):
                try:
...
def paletteGen(prompt, colors, seed, device, precision):
    ...
    for _ in txt2img(prompt, "", False, 1, False, False, int(width), 512, 1, 6, 7.0, {"apply":False}, {"hue":0, "tint":0, "saturation":50, "brightness":70, "contrast":50, "outline":50}, seed, 1, 512, device, precision, [{"file": "some/path/none", "weight": 0}], False, False, False, False, False, False):
        image = _[1]
